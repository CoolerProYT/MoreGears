...
<GearStats group="tools" />

::: details How these numbers work
- **Attack damage** includes the 1 damage every player deals with an empty hand, so it matches the tooltip.
...
<RecipeGrid tier="bronze" group="tools" />

### Steel {.tier .tier-steel}

<RecipeGrid tier="steel" group="tools" />
...
<RecipeGrid tier="titanium" group="tools" />

### Enderite {.tier .tier-enderite}

<RecipeGrid tier="enderite" group="tools" />
