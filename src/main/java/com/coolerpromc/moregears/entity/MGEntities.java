...
public class MGEntities {
    ...
    public static DeferredHolder<EntityType<?>, EntityType<MGArrowEntity>> registerArrow(String name, DeferredItem<MGArrowItem> item) {
        return ENTITIES.register(name, () -> EntityType.Builder.<MGArrowEntity>of((entityType, level) -> new MGArrowEntity(entityType, level, item.toStack(1)), MobCategory.MISC).sized(0.5f, 0.5f).clientTrackingRange(4).updateInterval(20).build(name));
    }
