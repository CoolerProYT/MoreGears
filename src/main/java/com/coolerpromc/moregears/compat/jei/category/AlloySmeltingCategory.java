...
public class AlloySmeltingCategory extends AbstractRecipeCategory<RecipeHolder<AlloySmeltingRecipe>> {
    ...
    public void draw(RecipeHolder<AlloySmeltingRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, 99, 30, 176, 18, 20, arrowHeight, 256, 256);

        int energyScaled = (int) Math.ceil((double) 1000 / 100000 * 58);
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, 9, 13 + (58 - energyScaled), 176, 101 - energyScaled, 14, energyScaled, 256, 256);

        int energyGeneration = (tickCount % 2000) * 18 / 2000;
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, 33, 44 + energyGeneration, 176, energyGeneration, 12, 18 - energyGeneration, 256, 256);
    }
